            ...
            <div class="search-body" ref="resultsContainerRef">
              ...
              <div v-else-if="results.length > 0" class="search-results">
                ...
                >
                  ...
                  <!-- Option 9: breadcrumb section path -->
...
// ─── Option 8: keyboard navigation through result cards ───────────────────────
...
// ─── Load Pagefind + Option 5: ranking tuning ─────────────────────────────────
onMounted(async () => {
  try {
    const importPath = '/pagefind/pagefind.js'
    pagefind = await new Function('path', 'return import(path)')(importPath)
    await pagefind.init()
    // Option 5: tune ranking for short single-clause pages
    await pagefind.options({
      ranking: {
        pageLength:     0.4,   // stop penalising longer clauses
        termFrequency:  0.8,   // slight bias toward raw term count
        termSimilarity: 1.2,   // prefer exact term matches
        termSaturation: 1.6,   // allow high-frequency terms to keep boosting rank
      }
    })
  } catch {
...
// ─── Option 10: debounced search input ───────────────────────────────────────
function debouncedSearch() {
  clearTimeout(searchTimer)
  searchTimer = setTimeout(doSearch, 200)
}

// ─── Option 7: exact-match boost + Option 5 ranking ─────────────────────────
async function doSearch() {
  ...
  try {
    ...
    // Option 7: exact phrase boost for multi-word queries
    ...
    if (query.value.trim().includes(' ')) {
      ...
      } catch { /* exact search optional — silently ignore if unsupported */ }
    }

    const allResults = await Promise.all(search.results.slice(0, 25).map(r => r.data()))

    // Exact matches float to the top
    results.value = [
...
/* ── Option 8: keyboard-focusable result cards ── */
.result-card {
